...
- The current position moves forward by the length plus the skip size, a total
  ...
  size increases to 2. The third length, 1, selects a sublist of a single
  element, and so reversing it has no effect.
...
class KnotHash:
    def __init__(self, elements, lenghts):
        self.knot = list(range(elements))
    ...
    def reverse_section(self, length):
        """Reverse a section of knot with length of `length`."""
        start, stop = self.section_indices(length)
        if start <= stop:
            section_slice = slice(start, stop + 1)
            section = self.knot[section_slice]
            self.knot[section_slice] = reversed(section)
        else:
            first_chunk = slice(start, None)
            second_chunk = slice(None, stop + 1)
            section = self.knot[first_chunk] + self.knot[second_chunk]
            reversed_section = list(reversed(section))
            elements_in_first_chunk = len(self.knot[first_chunk])
            self.knot[first_chunk] = reversed_section[:elements_in_first_chunk]
            self.knot[second_chunk] = reversed_section[
                elements_in_first_chunk:]

    def section_indices(self, length):
        """Returns a valid start and stop index for section based on length."""
        start = self.pos % self.elements
        stop = (self.pos + length - 1) % self.elements
        return start, stop

    def update_pos(self, step):
        self.pos += step + self.skip_step
...
class TestKnotHash(unittest.TestCase):
    def test_same_list_with_no_lengths(self):
        knot = KnotHash(256, [])
        self.assertEqual(knot.hash(), list(range(256)))

    def test_same_list_with_length_one(self):
        knot = KnotHash(256, [1])
        self.assertEqual(knot.hash(), list(range(256)))

    def test_reversed_knot_with_length_equal_to_elements(self):
        elements = 10
        knot = KnotHash(elements, [elements])
        reversed_knot = list(reversed(list(range(elements))))
        self.assertEqual(knot.hash(), reversed_knot)

    def test_sample_lenghts(self):
        lenghts = [3, 4, 1, 5]
