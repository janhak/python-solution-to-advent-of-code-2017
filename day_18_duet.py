...
(the value of a) is played. After that, a is set to 0, causing the subsequent
rcv and jgz instructions to both be skipped (rcv because a is 0, and jgz
because a is not greater than 0). Finally, a is set to 1, causing the next jgz
instruction to activate, jumping back two instructions to another jump, which
jumps again to the rcv, which ultimately triggers the recover operation. At the
time the recover operation is executed, the frequency of the last sound played
is 4.
...
class Duet:
    ...
    def set(self, reg, value):
        self.registers[reg] = self.v_or_r(value)

    def add(self, reg, value):
        self.registers[reg] += self.v_or_r(value)

    def snd(self, reg):
        self.last_played = self.registers[reg]
    ...
    def rcv(self, reg):
        if self.registers[reg]:
            raise ValueError('Sound played {}'.format(self.last_played))

    def jgz(self, reg, value):
        if self.v_or_r(reg) > 0:
            return self.v_or_r(value)

    def v_or_r(self, value):
        return value if isinstance(value, int) else self.registers[value]


def execute(instr, duet):
    i = 0
    while i < len(instr):
        ins_type, args = instr[i]
        ret = getattr(duet, ins_type)(*args)
        i += ret if ret else 1


if __name__ == '__main__':
    lines = lines_from_file('day_18_data.txt')
    instr = [line_to_instr(l) for l in lines]
    duet = Duet()
    execute(instr, duet)
